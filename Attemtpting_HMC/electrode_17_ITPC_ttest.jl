...
"""
function generate_null_distribution(n_permutations::Int=1000)
...
"""
Print only statistical significance and null hypothesis bounds
"""
function analyzeAndPrintPhases(phases::Vector{Complex{Float64}})
    # Calculate observed ITPC
    mean_phase = mean(phases)
    observed_itpc = abs(mean_phase)
    
    # Generate null distribution
    null_dist = generate_null_distribution(1000)
    
    # Calculate critical values
    critical_value_95 = quantile(null_dist, 0.95)
    
    # Calculate p-value
    p_value = mean(null_dist .>= observed_itpc)
    
    # Print only the statistical results
    println(@sprintf("Critical value (95%%): %.4f", critical_value_95))
    println(@sprintf("p-value: %.4f", p_value))
    println("Significant: ", p_value < 0.05 ? "Yes" : "No")
end
