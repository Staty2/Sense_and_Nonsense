...
"""
Print mean ITPC and statistical bounds
"""
function analyzeAndPrintPhases(phases::Vector{Complex{Float64}})
    ...
    # Calculate mean ITPC and determine significance
    itpc_values = abs.(phases)
    mean_itpc = mean(itpc_values)
    is_significant = mean_itpc > upper_bound
    
    # Print results in desired format
    println("Null hypothesis bounds: ", upper_bound, " - ", lower_bound)
    println("GN condition significant: ", is_significant)
    println("Mean ITPC: ", mean_itpc)
end
